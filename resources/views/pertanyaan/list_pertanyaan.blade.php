  ...
  <div class="col-md-12 mb-4">
    ...
    <div class="card card-small">
      <div class="card-header border-bottom">
        <h6 class="m-0">
          Tabel Pertanyaan
          <a href="{{ url('pertanyaan/tambah') }}" class="btn btn-success float-right" title="Tambah Data"><i class="fa fa-plus"></i> Tambah</a>
        </h6>
      </div>
...
<div class="modal fade" id="modalForm">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <form action="{{ url('pertanyaan/ubahPertanyaan') }}" method="POST" id="formEdit">
...
<script>
  $(document).ready(function() {
    var listPertanyaan = {!! json_encode($list_pertanyaan) !!};
        ...
        initDatatable1.row.add( [
            ...
            '<div>'
              +'<div class="btn-group">'
                ...
                /*+'<a href="'+ '{{url("pertanyaan/hapus")}}'+'/'+data.id +'" class="btn btn-sm btn-outline-danger" data-id="'+data.id+'" onclick="return confirm(\'Hapus data?\');" title="Hapus data"> <i class="fa fa-trash-alt"></i> </a>'*/
              +'</div>'
            +'</div>'
        ] ).draw( false );
      }
    ...
    $('.btnDelete').on("click", function(e) {
      ...
      if(proceedDel) {
        ...
        if(id) {
          prepDelete('{{ url("pertanyaan/hapusPertanyaan") }}', 'post', [
            {name: 'id', value: id}
          ]);
        }
      }
  ...
  function getDetail(id, callback) {
    if(id) {
      $.ajax({
        url: '{{ url("pertanyaan/getPertanyaan") }}',
